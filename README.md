...
The implemented message routes are Internet/libp2p relay, LAN, Wi-Fi Direct, Wi-Fi Aware, Bluetooth GATT, Bluetooth mesh, nearby audio, and explicit NFC tap transfer. Chat attachments preserve their MIME type and are encrypted end-to-end before transport; the current client limit is 512 KiB per attachment, so images, short videos, audio clips, PDFs, documents, and arbitrary files can be shared through the large-payload routes. Nearby audio, NFC, and mesh are intentionally limited to small messages. IR is transmitter-only on stock Android and UWB is a proximity/ranging assist; neither is incorrectly used as a message route. Live relay delivery and hardware interoperability still require the release test matrix. See [transport capabilities](docs/TRANSPORTS.md), [production status](docs/PRODUCTION_STATUS.md), and the [production verification audit](docs/PRODUCTION_VERIFICATION.md) for release gates.
