...
- TCP `4001` for libp2p
- UDP `4001` for QUIC
- TCP `8080` for local health/info checks (keep this private if possible)

The SSH user must be able to run Docker and `sudo mkdir`/`sudo chown` as used by the deployment workflow.
